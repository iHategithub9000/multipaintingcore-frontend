document.addEventListener('DOMContentLoaded', async () => {
	...
	async function render() {
		...
		for (let y = fy - 1;
			(((y - fy) * zoom) < height) && (y < grid.length); y++) {
			if (y < 0) {
				continue;
			}
			for (let x = fx - 1;
				(((x - fx) * zoom) < width) && (x < grid[y].length); x++) {
				if (x < 0) {
					continue;
				}
